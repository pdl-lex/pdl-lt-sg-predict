...
def _row_from_metadata(pkl_file: Path, meta: dict) -> dict:
    ...
    return {
        ...
        "top1_confidence": round(float(topk["top1_conf"]), 4) if topk.get("top1_conf") is not None else "–",
        "top3_confidence": round(float(topk["top3_conf"]), 4) if topk.get("top3_conf") is not None else "–",
